...
// Insert
export async function insertInto<T extends TablesInsert<any>>(
  tableName: string,
  record: T
): Promise<T | null> {
...
// Update
export async function updateById<T extends TablesUpdate<any>>(
  tableName: string,
  id: string,
  updates: T
): Promise<T | null> {
...
// --------------------------------------------------------
// ⬇⬇ TABLE-SPECIFIC APIs (PROFILES → REPORTS) ⬇⬇
// --------------------------------------------------------
...
export async function updateUserRole(
  userId: string,
  roleId: string,
  updates: TablesUpdate<'user_roles'>
) {
  const { data, error } = await supabase
    .from('user_roles')
    .update(updates)
...
export async function deleteUserRole(userId: string, roleId: string): Promise<boolean> {
  ...
  if (error) {
    console.error(
      `Error deleting user_role with user_id ${userId} and role_id ${roleId}:`,
      error
    );
    return false;
  }
